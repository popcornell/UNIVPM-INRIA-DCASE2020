...
class FixMatch(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        self.student = SEDBaseline(self.config, False) # student here is simply ema of teacher
        self.teacher = SEDBaseline(self.config, False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        strong_weak_augm_teach, weak_weak_augm_teach, logits_weak_augm_teach = self.teacher(weak_augm_feats.unsqueeze(1)) # weak augmented predictions

        # labeled loss
        loss = torch.zeros((1)).to(self.device) # ADD to lightining hooks.py

        strong_loss_teacher = 0
        if strong_mask.any(): # we have strong examples
            strong_loss_teacher = (self.weighted_strong(logits_weak_augm_teach[strong_mask], strong[strong_mask])).sum()
        ...
        strong_strong_augm, weak_strong_augm, logits_strong_augm = self.student(strong_augm_feats.unsqueeze(1))  # strong augmented prediction
        ...
        if self.tot_step % self.config["training"]["accumulate_batches"] == 0:
            ...
            tot_steps = len(self.train_dataloader())*self.config["training"]["n_epochs_lr_schedule"] // self.config["training"]["accumulate_batches"]
    ...
    def validation_step(self, batch, batch_indx):
        ...
        student_strong, student_weak, student_logits = self.student(mixture.unsqueeze(1))
        teacher_strong, teacher_weak, teacher_logits = self.teacher(mixture.unsqueeze(1))
    ...
    def configure_optimizers(self):
        ...
        self.opt = torch.optim.Adam(self.teacher.parameters(), self.config["opt"]["lr"], betas=(0.9, 0.999), weight_decay=self.config["opt"]["weight_decay"])
