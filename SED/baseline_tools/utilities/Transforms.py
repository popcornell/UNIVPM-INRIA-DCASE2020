...
class SpliceFeats(Transform):

    def __init__(self, context_size, subsampling):
        self.context_size = context_size
        self.subsampling = subsampling


    def transform_data(self, Y):
        ...
        Y_pad = np.pad(
            Y,
            [(self.context_size, self.context_size), (0, 0)],
            'constant')

        Y_spliced = np.lib.stride_tricks.as_strided(
            Y_pad,
            (Y.shape[0], Y.shape[1] * (2 * self.context_size + 1)),
            (Y.itemsize * Y.shape[1], Y.itemsize), writeable=False)

        orig_len = len(Y_spliced)
        Y_ss = Y_spliced[::self.subsampling]
        Y_ss = Y_ss[:int(orig_len /self.subsampling)]

        return torch.from_numpy(Y_ss[None, ...])
...
def get_transforms(frames=None, scaler=None, add_axis=0, noise_dict_params=None, combine_channels_args=None, splice=None):
    transf = []
    unsqueeze_axis = None
    ...
    if splice is not None:
        transf.append(SpliceFeats(splice[0], splice[1]))
