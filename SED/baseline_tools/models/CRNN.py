...
class CRNN(nn.Module):
    ...
    def __init__(self, n_in_channel, nclass, attention=False, activation="Relu", dropout=0,
                 train_cnn=True, rnn_type='BGRU', n_RNN_cell=64, n_layers_RNN=1, dropout_recurrent=0, adversarial=False,  **kwargs):
    ...
    def forward(self, x):
        ...
        #x = (x - torch.mean(x, dim=([x for x in range(len(x.shape)) if x != 0]), keepdim=True)) / (torch.std(x, dim=([x for x in range(len(x.shape)) if x != 0]), keepdim=True) + 1e-8) #instance norm
        ...
        if self.adversarial:
            return strong, weak, logits_strong, adversarial
        else:
            return strong, weak, logits_strong


if __name__ == '__main__':
    import torch 
    inp = torch.rand((2, 1, 16000, 64))
    c = CRNN(1, 10, kernel_size=[3, 3, 3], padding=[1, 1, 1], stride=[1, 1, 1], nb_filters=[64, 64, 64],
         pooling=[(1, 4), (1, 4), (1, 4)])
    import ipdb
    ipdb.set_trace()
    out = c(inp)
