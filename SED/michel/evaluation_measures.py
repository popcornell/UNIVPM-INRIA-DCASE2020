...
def compute_sed_eval_metrics(predictions, groundtruth):
    ...
    results = metric_event.results_class_wise_average_metrics()
    print(' || '.join([f"{k}: {100*v:.2f}%" for k, v in results['f_measure'].items()]))
    print(' || '.join([f"{k}: {100*v:.2f}%" for k, v in results['error_rate'].items()]))
