...
class CRNN(nn.Module):
    ...
    def __init__(self, n_in_channel, nclass, attention=False, activation="Relu", dropout=0,
        ...
        self.trainable_pcen = []
        self.in_f_size = (1, 128, 628)
        self.use_s = True
        self.s = [0.015, 0.08]
        self.eps = 1e-6
        self.n_pcen = 2 # Number of pcen transforms in layer
        self.normalize_pcen_stack = False

        if self.use_pcen:
         # print("Will use new pcen stack")
            self.trainable_pcen = PCENstack(self.n_pcen,
                                            self.in_f_size[-2],
                                            self.s,
                                            self.eps,
                                            self.normalize_pcen_stack)
        # End of PCEN--------------------
    ...
    def load_state_dict(self, state_dict, strict=True):
        self.trainable_pcen.load_state_dict(state_dict["pcen"])
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        state_dict = {"pcen": self.trainable_pcen.state_dict(destination=destination, prefix=prefix, keep_vars=keep_vars),
    ...
    def save(self, filename):
        parameters = {'pcen': self.trainable_pcen.state_dict(),
    ...
    def forward(self, x):
        ...
        # ------------   PCEN trainable features
        if self.trainable_pcen is not None:
            # print("INPUT BEFORE NORM:", torch.min(x), torch.max(x))
            x = x.permute(0,1,3,2)
            x = self.trainable_pcen(x)
            x = x.permute(0,1,3,2)
            # print("PCEN STATS AFTER NORM:", torch.min(x), torch.max(x))
        # ------------   End of PCEN trainable features ------------------------------
